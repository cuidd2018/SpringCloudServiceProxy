...
public final class BeanUtils {
    ...
    public static Object getProperty(Object source, Field field)
        ...
        Method _targetReadMethod = propertyDescriptor.getReadMethod();
        Object targetObj = _targetReadMethod.invoke(source);
        return targetObj;
    }
        ...
        if (fields != null) for (int i = 0; i < fields.size(); i++) {
            try {
                ...
                if (!Modifier.isStatic(field.getModifiers()) && !Modifier.isFinal(field.getModifiers())) {
                    ...
                    PropertyDescriptor _target = new PropertyDescriptor(field.getName(), target.getClass());
                    PropertyDescriptor _source = new PropertyDescriptor(field.getName(), source.getClass());
                    // 调用getter
                    Method readMethod = _source.getReadMethod(); // getName()
                    Object obj = readMethod.invoke(source);

                    if (skipSourceNull && obj == null) {
                        continue;
                    }

                    Method _targetReadMethod = _target.getReadMethod();
                    Object targetObj = _targetReadMethod.invoke(target);
                    if (skipTargetHave && targetObj != null) {
                        continue;
                    }
                    // 调用setter
                    Method writeMethod = _target.getWriteMethod(); // setName()
